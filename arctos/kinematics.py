...
Still in development - needs additional unit tests.
Worked on by Gavin Prescott.
"""
...
class Kinematics:
    def __init__(self, dh_params: list[list[float]]):
        table = np.array(dh_params, dtype=float)
        if table.shape != (6, 4):
            raise ValueError(f"dh_params must be shape (6, 4), got {table.shape}")
        self.dh_table = table
    ...
    @staticmethod
    def _dh_transform(alpha: float, a: float, theta: float, d: float) -> np.ndarray:
    ...
    @staticmethod
    def _dh_end_effector_transform(T: np.ndarray) -> Pose:
        """Extract end effector pose (x, y, z, roll, pitch, yaw) from the final transformation matrix."""
        x = T[0, 3]
        y = T[1, 3]
        z = T[2, 3]

        # Extract rotation angles (assuming Z-Y-X order)
    ...
    def inverse(self, target_pose: Pose) -> dict[str, list[float]]:
        """Given a target end effector pose, return the required joint angles (degrees).
        
        Input:
        - target_pose: Pose object containing desired x, y, z, roll, pitch, yaw of the end effector.

        Output:
        - dict with keys 'joint_angles' containing a list of 6 joint angles in degrees that achieve the target pose.
        """
...
if __name__ == "__main__":
    try:
        ...
        # If this file is executed directly, the package root may not be on sys.path.
    ...
    print(f"Roll: {end_effector_pose.roll:.3f} deg")
    print(f"Pitch: {end_effector_pose.pitch:.3f} deg")
    print(f"Yaw: {end_effector_pose.yaw:.3f} deg")
